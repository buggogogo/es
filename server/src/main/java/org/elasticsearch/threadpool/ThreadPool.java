...
public class ThreadPool implements Scheduler {
    ...
    //创建线程池
    //1.创建builder 的配置对象，FixedExecutorBuilder,ScalingExecutorBuilder等，然后将这些对象返回，并遍历调用每个对象的build方法，创建真正的线程池
    public ThreadPool(final Settings settings, final ExecutorBuilder<?>... customBuilders) {
        ...
        //检查，防止已经定义过的线程池，在本函数中再次创建（检查是否builders 有重名）
        ...
        //build线程池，并检查是否重名（检查builders的values对象赋值是否正确）
